...
export const uploadChatAttachment = async ({ file, token, publicId, onProgress, originalFileName, configOverride }) => {
    if (!file) {
    ...
    try {
        ...
        if (config.mode !== 'preset') {
            signaturePromise = axios.get(`${API_URL}/api/chat/media/signature`, {
                params: {
        ...
        const signatureData = signaturePromise ? await signaturePromise : null;
